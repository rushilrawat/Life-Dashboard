...
either local storage or a directly-connected service via a hand-written
adapter, one adapter per service, each exposing a small fixed menu of
capabilities. No LLM in the data path, no Anthropic API cost per sync,
that tradeoff was made deliberately, see the Sync section of
`docs/ARCHITECTURE.md` for why. Read every file in `docs/` before
writing code. They are the spec, not background reading, follow them
exactly unless they conflict with something below.
...
- Minimal Express backend, one job: hold every third-party API key
  server-side and run each connected service's adapter, so no key of
  any kind ever reaches the browser bundle
...
- One hand-written adapter per connected service (GitHub first), each
  exposing a small fixed menu of capabilities, not a free-text query.
  This replaced an earlier MCP-plus-Anthropic-API design, on purpose,
  to get sync cost to effectively zero. Don't reintroduce an LLM into
  the sync path, or bring MCP back, without the same kind of explicit
  conversation that removed it, that decision doesn't get quietly
  reversed a second time either.
- Batch every api-sourced block's adapter call into one sync request,
  resolved server-side, in parallel. Never one round trip per block
  from the frontend.
- Never call a third-party API, or hold any API key, from browser
  code. Every adapter runs server-side, every key lives in `.env`, the
  frontend only ever talks to the local backend.
